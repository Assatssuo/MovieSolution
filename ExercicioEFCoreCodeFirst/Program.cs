    ...
    class Program
    {
        static void Main(string[] args)
        {
            using (var db = new MovieContext())
            {
                ...
                {
                    Console.WriteLine("{0} \t {1}", a.Name, a.DateBirth);
                }
                Console.WriteLine();

                Console.WriteLine("a) Listar o elenco de um determinado filme");
                Console.ReadKey();
                foreach (Movie m in db.Movies)
                {
                    var query =
                    from actorMovie in db.Characters
                    where actorMovie.MovieId == m.ID
                    select new
                    {
                        CharacterName = actorMovie.Character,
                        ActorName = actorMovie.Actor.Name
                    };

                    Console.WriteLine("Movie \t {0}", m.Title);
                    foreach (var a in query)
                    {
                        Console.WriteLine("Actor: {0} \t Character: {1}", a.ActorName,a.CharacterName);
                    }
                }
                Console.ReadKey();
                Console.WriteLine();
